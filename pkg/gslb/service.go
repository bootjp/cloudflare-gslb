...
func (s *Service) checkOrigin(ctx context.Context, origin config.OriginConfig, checker healthcheck.Checker) {
	...
	if !exists {
		status = &OriginStatus{
			UsingPriority:   len(origin.PriorityFailoverIPs) > 0,
...
func (s *Service) replaceUnhealthyRecord(ctx context.Context, origin config.OriginConfig, unhealthyRecord cf.DNSRecord) error {
	...
	// 優先IPを使用中でない場合、または優先IPが異常で通常のフェイルオーバーIPがない場合
	...
	if len(origin.FailoverIPs) > 0 {
		...
		// IPタイプをチェック
		if origin.RecordType == "A" && net.ParseIP(newIP).To4() == nil {
			return fmt.Errorf("failover IP %s is not a valid IPv4 address for A record", newIP)
		} else if origin.RecordType == "AAAA" && net.ParseIP(newIP).To4() != nil {
			return fmt.Errorf("failover IP %s is not a valid IPv6 address for AAAA record", newIP)
		}
	...
	// フェイルオーバーIPが設定されていない場合は、従来の方法でIPを生成
	...
	if origin.RecordType == "A" {
		...
		ipv4[3]++
		newIP = ipv4.String()
	} else if origin.RecordType == "AAAA" {
